...
pc.autoscale = function() {
  ...
  var defaultScales = {
    ...
    "number": function(k) {
	  ...
	  var m = 5;
      var span = extent[1] - extent[0], step = Math.pow(10, Math.floor(Math.log(span / m) / Math.LN10)), err = m / span * step;
      if (err <= .15) step *= 10; else if (err <= .35) step *= 5; else if (err <= .75) step *= 2;
      if(Math.ceil(extent[0] / step) * step == extent[0]){
		extent[0] = Math.ceil(extent[0] / step) * step;
	  }else{
		extent[0] = Math.ceil(extent[0] / step - 1) * step;
	  }
	  if(extent[1] == Math.floor(extent[1] / step) * step){
		extent[1] = Math.floor(extent[1] / step) * step;
	  }else{
		extent[1] = Math.floor(extent[1] / step + 1) * step;
	  }
	  only_use_in_parallel_step[only_use_in_parallel_count] = {};
	  only_use_in_parallel_step[only_use_in_parallel_count].floor = extent[0];
	  only_use_in_parallel_step[only_use_in_parallel_count].ceil = extent[1];
	  only_use_in_parallel_step[only_use_in_parallel_count].step = step;
	  only_use_in_parallel_count++;
	  only_use_in_parallel_step.length = only_use_in_parallel_count;
...
pc.axisDots = function() {
	...
	__.data.forEach(function(d) {
		__.dimensions.map(function(p, i) {
			ctx.fillRect(position(p) - 2, yscale[p](d[p]) + 22, 5, 5);
		});
	});
	return this;
};
...
pc.clear = function(layer) {
  ctx[layer].clearRect(0,0,w()+2,h()+50);
  return this;
};
...
pc.createAxes = function() {
  ...
  g = pc.svg.selectAll(".dimension")
    ...
    .enter().append("svg:g")
      ...
      .attr("transform", function(d) { return "translate(" + xscale(d) + "," + 24 + ")"; });
...
pc.updateAxes = function() {
  ...
  g_data.enter().append("svg:g")
      .attr("class", "dimension")
      .attr("transform", function(p) { return "translate(" + position(p) + "," + 24 + ")"; })
      .style("opacity", 0)
    .append("svg:g")
      .attr("class", "axis")
      .attr("transform", "translate(0,0)")
